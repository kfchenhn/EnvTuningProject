...
class ExecutionManager:
    ...
    def execute_function_calls(self, tool_content: str, state: InstanceState, instance_id: str, entry_id: str) -> ExecutionResult:
        ...
        try:
            ...
            #######新增（开始）#######
            # 记录本次尝试到轨迹回放池，为后见诊断准备输入
            self.record_attempt(state, decoded_responses, execution_results, has_error)
            #######新增（结束）#######
    ...
    #######新增（开始）#######
    def record_attempt(self, state: InstanceState, decoded_responses: List[Any], execution_results: List[Any], has_error: bool) -> None:
        """
        记录单次尝试到轨迹回放池
        
        Args:
            state: 实例状态
            decoded_responses: 解码后的响应
        ...
        state.single_turn_attempt_records.append(attempt)
    #######新增（结束）#######
    ...
    def format_execution_response(self, execution_results: List[Any], has_error: bool, blind_mode: bool = False) -> Tuple[str, float]:
        ...
        Args:
            ...
            has_error: 是否有错误
            blind_mode: 是否启用盲盒反馈模式（静默收集，不返回增强提示）
        ...
        #######新增（开始）#######
        # 盲盒反馈模式：只返回执行结果和继续/结束格式约束，不返回可操作增强提示
        if blind_mode:
            user_hint = (
                f"Here are the function's execution results. Execution results:{response_content}\n "
                f"You should continue to call functions until fulfilling user's request."
            )
