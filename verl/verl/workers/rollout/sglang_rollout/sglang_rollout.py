...
class SGLangRollout(BaseRollout):
    ...
    async def _async_rollout_a_request(
        ...
        **kwargs,
        ...
        # current_turns 关注的是助手侧的响应次数，更侧重于模型生成的频率和上限控制。
        # user_turns 关注的是用户侧的回复次数，更侧重于外部交互的频率和奖励计算
        while current_turns < self.config.multi_turn.max_assistant_turns:
            
            # 如果当前状态为等待状态
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                # PENDING 状态用于初始化工具（调用 tool.create）和启动交互（调用 interaction.start_interaction），准备资源后切换到 RUNNING 状态
                await self._handle_pending_state(_req)
                _req.state = AsyncRolloutRequestStateEnum.RUNNING
            # 如果当前状态为运行状态，并且上一个消息包含工具调用
            elif _req.state == AsyncRolloutRequestStateEnum.TOOL_CALLING:
                # TOOL_CALLING 状态用于执行工具调用：解析并执行 tool_calls，添加工具响应消息，更新指标，然后切换回 RUNNING
                if _req.messages[-1].tool_calls is not None:
                    parsed_tool_calls = _req.messages[-1].tool_calls
                    
                    # 输出工具调用的结果、奖励和指标，并将工具调用的结果添加到消息中
                ...
                # RUNNING 状态用于生成模型响应：调用引擎生成文本，检查长度和结束原因，解析工具调用或交互需求，然后添加助手消息并切换状态（TOOL_CALLING、INTERACTING 或 COMPLETED）
                ...
                if finish_reason_type == FinishReasonTypeEnum.LENGTH:
                    ...
                    if self._function_call_parser and self._function_call_parser.has_tool_call(content):
                        ...
                        if _req.interaction_kwargs and user_turns < self.config.multi_turn.max_user_turns and current_turns < self.config.multi_turn.max_assistant_turns:
                            ...
                            break
            # 如果当前状态为交互状态，说明上一个消息包含了模型与外部环境交互（模拟用户输入）的需求，此时需要调用interaction模块生成用户的回复，并将回复添加到消息中
            elif _req.state == AsyncRolloutRequestStateEnum.INTERACTING:
                            
            # INTERACTING 状态用于与外部环境交互：调用 interaction.generate_response 生成用户回复，添加用户消息，记录奖励，然后切换回 RUNNING 或 COMPLETED。
                user_turns += 1
                
                messages = [{"role": x.role, "content": x.content} for x in _req.messages]
                # 调用interaction模块生成用户回复，interaction模块会根据传入的消息和交互参数与外部环境进行交互，并返回用户的回复、是否结束对话的标志、奖励和指标等信息
        ...
        # 该函数的设计初衷是为了同时计算工具的奖励和释放工具资源，避免在工具执行过程中占用过多资源导致后续工具调用失败
        async def calc_reward_and_release_fn(name: str, tool: BaseTool):
            reward = await tool.calc_reward(_req.request_id, **_req.tools_kwargs[name].get("calc_reward_kwargs", {}))
            await tool.release(_req.request_id, **_req.tools_kwargs[name].get("release_kwargs", {}))
            return name, reward
        # 这里的实现是串行计算每个工具的奖励和释放资源，如果工具的calc_reward和release函数都比较快，这样实现可以保证代码逻辑简单清晰；如果工具的calc_reward和release函数比较慢，可以考虑并行计算奖励和释放资源，但需要注意并行计算时可能会占用过多资源导致后续工具调用失败，因此需要根据实际情况进行权衡和调整
        tool_reward_tasks = []
